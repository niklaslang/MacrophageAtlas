...
### path variables ###
DA.path <- "/home/s1987963/ds_group/Niklas/all_kidney/DA/"
...
######################################
### subset data ###
kidney <- subset(kidney, subset = study == "wilson_kidney") # 23034 cells
kidney <- subset(kidney, subset = lineage_integrated != "Doublet") # 23034 cells
...
# MP subset
MP <- subset(kidney, subset = lineage_integrated== "MP")

# add metadata for visualisation
cell.data <- data.table(barcode = colnames(kidney),
                        celltype = Idents(kidney),
                        lineage = kidney$lineage_integrated)
...
kidney <- AddMetaData(kidney, cell.data, col.name = "celltype_ordered")

# reorder lineage factors
kidney$celltype_ordered = factor(kidney$celltype_ordered)
kidney$celltype_ordered <- factor(kidney$celltype_ordered, levels(kidney$celltype_ordered)[c(2,1,3)])

# reorder patient.ID factors
MP$patient.ID <- factor(MP$patient.ID)
...
n.clusters <- length(kids <- set_names(levels(MP.sce$Celltype)[c(53,54)]))
...
n_cells <- table(MP.sce$Celltype, MP.sce$Patient.ID)[c(53,54),]
...
# set sample ID levels
df.celltypes$Sample.ID <- factor(df.celltypes$Sample.ID, c("GSM3823939_control", "GSM3823940_control", "GSM3823941_control",
                                                           "GSM3823942_diabetes", "GSM3823943_diabetes", "GSM3823944_diabetes"))

# add healthy no.1 with zero MPs
df.celltypes <- rbind(df.celltypes, c(as.numeric(0), "Combined Kidney Monocyte", "GSM3823939_control", "healthy"))
df.celltypes <- rbind(df.celltypes, c(as.numeric(0), "Combined Kidney Macrophage", "GSM3823939_control", "healthy"))

# reorder lineage factors
df.celltypes$Celltype <- factor(df.celltypes$Celltype, levels(df.celltypes$Celltype)[c(2,1)])

# create celltype composition data table: healthy vs fibrotic
df.MP.fibrotic <- df.celltypes[df.celltypes$Condition == "fibrotic",]
df.MP.fibrotic <- rbind(df.MP.fibrotic, df.MP.fibrotic)
df.MP.fibrotic$Pair.ID <- rep(1:6, each = 2)
df.MP.fibrotic$Pair.ID <- paste0(df.MP.fibrotic$Pair.ID, "_", rep(1:2, 6))
...
df.MP.healthy <- rbind(df.MP.healthy, df.MP.healthy, df.MP.healthy)
df.MP.healthy$Pair.ID <- rep(1:6, each = 2)
df.MP.healthy$Pair.ID <- paste0(df.MP.healthy$Pair.ID, "_", rep(1:2, 6))
...
# plot UMAP
MP.celltype.umap <- DimPlot(kidney, reduction = "umap", group.by = "celltype_ordered", cols = c(seurat.colours,"#999999"), label = TRUE, repel = TRUE, pt.size = 0.1) +
...
MP.celltype.boxplot <- ggplot(abundance.pairs, aes(x = log.ratio, y = Celltype, fill = Celltype)) +
  ...
  geom_segment(aes(x=-0.5, y=2.5, xend=-3, yend=2.5), lineend = "round", linejoin = "mitre", arrow=arrow(), size=0.1) + 
  geom_segment(aes(x=0.5, y=2.5, xend=3, yend=2.5), lineend = "round", linejoin = "mitre", arrow=arrow(), size=0.1) +
...
MP.figure <- MP.celltype.umap + MP.celltype.barplot + MP.celltype.boxplot + plot_layout(widths = c(4,1,1), guides = 'collect')
