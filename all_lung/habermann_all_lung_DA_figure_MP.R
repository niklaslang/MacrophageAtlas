...
######################################
### subset data ###
# only use habermann data for downstream analyses
lung <- subset(lung, subset = study == "habermann_lung")
lung <- subset(lung, subset = lineage_integrated != "Red blood cell" & lineage_integrated != "Doublet") # 153401 cells
# MP subset
MP <- subset(lung, subset = lineage_integrated == "MP") # 58025 cells
...
df.MP.fibrotic$Pair.ID <- paste0(df.MP.fibrotic$Pair.ID, "_", rep(1:12, 20))
...
df.MP.healthy <- rbind(df.MP.healthy, df.MP.healthy)
df.MP.healthy$Pair.ID <- rep(1:20, each = 12)
df.MP.healthy$Pair.ID <- paste0(df.MP.healthy$Pair.ID, "_", rep(1:12, 20))
...
png(paste0(DA.path, "habermann_celltype_umap.png"), width=1000,height=1000,units="px")
...
png(paste0(DA.path, "habermann_celltype_barplot.png"), width=500,height=1000,units="px")
...
MP.celltype.boxplot <- ggplot(abundance.pairs, aes(x = log.ratio, y = Celltype, fill = Celltype)) +
  ...
  theme(axis.text.x = element_text(size = 16),
        ...
        legend.position="none") +
  geom_text(x = 5, y = 10.90, label = "**", size = 10) +
  geom_text(x = 5, y = 5.90, label = "*", size = 10) +
  geom_text(x = 5, y = 2.90, label = "*", size = 10) +
  geom_text(x = 4.8, y = 1.90, label = "***", size = 10) +
  png(paste0(DA.path, "habermann_celltype_boxplot.png"), width=500,height=1000,units="px")
...
png(paste0(DA.path, "habermann_figure.png"), width=1700,height=800,units="px")
print(MP.figure)
dev.off()
