...
cell.data <- data.table(barcode = colnames(liver),
                        celltype = liver$celltype_integrated)
# meta cell no.1: monocyte-derived Macrophage
cell.data[, meta.celltype := ifelse(celltype %in% c("Combined Liver Macrophage 1"),
                                    "Combined Liver monocyte-derived Macrophage", paste0(celltype))]
# meta cell no.2: tissue resident Macrophage
cell.data[, meta.celltype := ifelse(celltype %in% c("Combined Liver KC1",
                                                    "Combined Liver KC2", 
                                                    "Combined Liver KC3",
                                                    "Combined Liver KC4"),
                                    "Combined Liver tissue resident Macrophage", paste0(meta.celltype))]
# meta cell no.3: Monocyte
cell.data[, meta.celltype := ifelse(celltype %in% c("Combined Liver Monocyte 1",
                                                    "Combined Liver Monocyte 2"),
                                    "Combined Liver Monocyte", paste0(meta.celltype))]
# meta cell no.4: cDC
cell.data[, meta.celltype := ifelse(celltype %in% c("Combined Liver cDC1",
                                                    "Combined Liver cDC2"),
                                    "Combined Liver cDC", paste0(meta.celltype))] 
cell.data <- data.frame(cell.data, row.names = cell.data$barcode)
cell.data$barcode <- NULL
cell.data$celltype <- NULL
liver <- AddMetaData(liver, cell.data, col.name = "meta.celltype")

### convert to sce ###
liver.sce <- as.SingleCellExperiment(liver)

### overview ###
table(liver.sce$meta.celltype, liver.sce$condition)

# creating pseudo-bulk samples
liver.pseudobulk <- aggregateAcrossCells(liver.sce, id=colData(liver.sce)[,c("meta.celltype", "patient.ID")])

# label meta cell types
label1 <- "Combined Liver monocyte-derived Macrophage"
label2 <- "Combined Liver tissue resident Macrophage"
label3 <- "Combined Liver Monocyte"
label4 <- "Combined Liver cDC"
...
## prepare data ##
# subset data
pseudobulk.cell <- liver.pseudobulk[,label1==liver.pseudobulk$meta.celltype]
...
summary(decideTests(res)) #  0 down and 0 up
top.genes <- topTags(res, n=7499)
write.csv(top.genes$table, file = paste0(DE.path, "DE_combined_liver_META_monocyte_derived_macrophages.csv"))
...
monocyte_derived_macrophage.plot1 <- ggplot(monocyte_derived_macrophage.genes_ordered, aes(x=logFC, y=-log10(p.adj))) +
  ...
  ggtitle("Liver monocyte-derived macrophage DE genes\nhealthy vs. fibrotic") +
...
png(paste0(DE.path, "liver_META_monocyte_derived_macrophage_volcano_1.png"), width=1200,height=800,units="px")
...
png(paste0(DE.path, "liver_META_monocyte_derived_macrophage_volcano_2.png"), width=1200,height=800,units="px")
...
## prepare data ##
# subset data
pseudobulk.cell <- liver.pseudobulk[,label2==liver.pseudobulk$meta.celltype]
...
# test for differences 
res <- glmQLFTest(fit, coef=ncol(design))
summary(decideTests(res)) # 10 down and 16 up
top.genes <- topTags(res, n=7491)
write.csv(top.genes$table, file = paste0(DE.path, "DE_combined_liver_META_tissue_resident_macrophage.csv"))
...
tissue_resident_macrophage.plot1 <- ggplot(tissue_resident_macrophage.genes_ordered, aes(x=logFC, y=-log10(p.adj))) +
  ...
  ggtitle("Liver tissue resident macrophages DE genes\nhealthy vs. fibrotic") +
  xlab(bquote(~Log[2]~ "fold change")) +
  ylab(bquote(~-Log[10]~adjusted~italic(P))) +
  theme_classic() +
  theme(axis.text.x = element_text(size = 16),
  ...
  geom_vline(xintercept=c(-logFC.cutoff, logFC.cutoff), linetype="longdash", colour="black", size=0.4) +
  geom_hline(yintercept=-log10(p.adj.cutoff), linetype="longdash", colour="black", size=0.4)
png(paste0(DE.path, "liver_META_tissue_resident_macrophage_volcano_1.png"), width=1200,height=800,units="px")
...
tissue_resident_macrophage.plot2 <- EnhancedVolcano(top.genes$table,
                                                    ...
                                                    xlim = c(-5,5),
                                                    ylim = c(0,2.5),
                                                    title = NULL,#'DE genes in liver tissue resident macrophages',
                                                    subtitle = NULL,#'healthy versus fibrotic',
                                                    cutoffLineType = 'twodash',
                                                    cutoffLineWidth = 0.8,
                                                    pointSize = 2,
                                                    ...
                                                    gridlines.major = FALSE,
                                                    gridlines.minor = FALSE)
png(paste0(DE.path, "liver_META_tissue_resident_macrophage_volcano_2.png"), width=1200,height=800,units="px")
...
## prepare data ##
# subset data
pseudobulk.cell <- liver.pseudobulk[,label3==liver.pseudobulk$meta.celltype]
...
res <- glmQLFTest(fit, coef=ncol(design))
summary(decideTests(res)) # 0 down and 0 up
top.genes <- topTags(res, n=8102)
write.csv(top.genes$table, file = paste0(DE.path, "DE_combined_liver_META_monocyte.csv"))
...
monocyte.plot1 <- ggplot(monocyte.genes_ordered, aes(x=logFC, y=-log10(p.adj))) +
  ...
  scale_y_continuous(limits = c(0,2)) +
  ggtitle("Liver monocyte DE genes\nhealthy vs. fibrotic") +
  xlab(bquote(~Log[2]~ "fold change")) +
  ylab(bquote(~-Log[10]~adjusted~italic(P))) +
  theme_classic() +
  theme(axis.text.x = element_text(size = 16),
  ...
  geom_vline(xintercept=c(-logFC.cutoff, logFC.cutoff), linetype="longdash", colour="black", size=0.4) +
  geom_hline(yintercept=-log10(p.adj.cutoff), linetype="longdash", colour="black", size=0.4)
png(paste0(DE.path, "liver_META_monocyte_volcano_1.png"), width=1200,height=800,units="px")
...
monocyte.plot2 <- EnhancedVolcano(top.genes$table,
                                  ...
                                  xlim = c(-2.5,2.5),
                                  ylim = c(0,2),
                                  title = NULL,#'DE genes in liver monocytes',
                                  subtitle = NULL,#'healthy versus fibrotic',
                                  cutoffLineType = 'twodash',
                                  cutoffLineWidth = 0.8,
                                  pointSize = 2,
                                  ...
                                  gridlines.major = FALSE,
                                  gridlines.minor = FALSE)
png(paste0(DE.path, "liver_META_monocyte_volcano_2.png"), width=1200,height=800,units="px")
...
## prepare data ##
# subset data
pseudobulk.cell <- liver.pseudobulk[,label4==liver.pseudobulk$meta.celltype]
...
# test for differences 
res <- glmQLFTest(fit, coef=ncol(design))
summary(decideTests(res)) # 1 down and 0 up
top.genes <- topTags(res, n=6633)
write.csv(top.genes$table, file = paste0(DE.path, "DE_combined_liver_META_cDC.csv"))
...
cDC.plot1 <- ggplot(cDC.genes_ordered, aes(x=logFC, y=-log10(p.adj))) +
  ...
  geom_vline(xintercept=c(-logFC.cutoff, logFC.cutoff), linetype="longdash", colour="black", size=0.4) +
  geom_hline(yintercept=-log10(p.adj.cutoff), linetype="longdash", colour="black", size=0.4)
png(paste0(DE.path, "liver_META_cDC_volcano_1.png"), width=1200,height=800,units="px")
print(cDC.plot1)
dev.off()
...
cDC.plot2 <- EnhancedVolcano(top.genes$table,
                             ...
                             gridlines.major = FALSE,
                             gridlines.minor = FALSE)
png(paste0(DE.path, "liver_META_cDC_volcano_2.png"), width=1200,height=800,units="px")
print(cDC.plot2)
dev.off()