...
logtransform.path <- "/home/s1987963/ds_group/Niklas/ramacha_liver/logtransform/"
...
saveRDS(liver.logtransform, paste0(logtransform.path, "ramacha_liver_healthy_logtransform.rds"))
...
### clustering ###
# evaluate different numbers of PCs and resolutions
dims <- c(6,8,11,14,18) # uncorrected/adjusted
...
# uncorrected/adjusted: 11first PCs, resolution 0.7
# adjusted for nFeatures: XY first PCs, resolution XY
# adjusted for nCounts + percent.mito: XY first PCs, resolution XY
liver.logtransform <- FindNeighbors(liver.logtransform, dims = 1:11)
liver.logtransform <- FindClusters(liver.logtransform, resolution = 0.7)
# add UMAP
liver.logtransform <- RunUMAP(liver.logtransform, dims=1:11, seed.use=1)

### save data ###
saveRDS(liver.logtransform, paste0(logtransform.path, "ramachandran_liver_healthy_logtransform.rds"))

## explore clustering at patient level ##
patient.clustering <- DimPlot(liver.logtransform, group.by = "seurat_clusters", split.by = "patient.ID", pt.size = 0.2, ncol = 7)
png(paste0(logtransform.path,"patient.clustering.png"), width=1600,height=500,units="px")
...
# monocyte markers
monocyte.genes <- c("CD14", "FCGR3A", "MNDA", "S100A8","S100A9")
...
macrophage.markers <- FeaturePlot(liver.logtransform, features = macrophage.genes, pt.size = 0.5, ncol = 5) & 
  scale_colour_gradientn(colours = rev(brewer.pal(n = 11, name = "RdYlBu")))
...
monocyte.markers <- FeaturePlot(liver.logtransform, features = monocyte.genes, pt.size = 0.5, ncol = 5) & 
  scale_colour_gradientn(colours = rev(brewer.pal(n = 11, name = "RdYlBu")))
png(paste0(logtransform.path,"monocyte.markers.png"), width=1500,height=500, units="px")
print(monocyte.markers)
dev.off()

# feature plot with DC markers
dc.markers <- FeaturePlot(liver.logtransform, features = dc.genes, pt.size = 0.5, ncol = 3) & 
  scale_colour_gradientn(colours = rev(brewer.pal(n = 11, name = "RdYlBu")))
...
liver.markers <- FeaturePlot(liver.logtransform, features = liver.genes, pt.size = 0.5, ncol = 4) & 
  scale_colour_gradientn(colours = rev(brewer.pal(n = 11, name = "RdYlBu")))
...
lineage.markers <- FeaturePlot(liver.logtransform, features = lineage.genes, pt.size = 0.5, ncol = 4) & 
  scale_colour_gradientn(colours = rev(brewer.pal(n = 11, name = "RdYlBu")))
