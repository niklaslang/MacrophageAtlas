...
harmony.samples.path <- "/home/s1987963/ds_group/Niklas/MP_lung/harmonize_samples/HVGs_overall/"

### load data ###
MP <- readRDS(MP.path) #95,936 cells
...
#dims <- c(31,39,47,50) #HVGs_conditions
dims <- c(50,44,37,32) # HVGs_overall
for(d in dims){
  ...
  dataset.batch.plot <- DimPlot(MP.harmony, reduction = "umap", group.by = "study", pt.size = 0.1)
  ...
  batch4.plot <- DimPlot(MP.harmony, reduction = "umap", group.by = "study", pt.size = 0.01)
  png(paste0(dim.path, "UMAP_dim", d, ".study.batch1.png"), width=1600, height=1000, units="px")
  print(batch4.plot)
  dev.off()
  
  # no.5 - split by dataset
  batch5.plot <- DimPlot(MP.harmony, reduction = "umap", split.by = "study", pt.size = 0.01, ncol = 3)
...
MP.harmony <- FindClusters(MP.harmony, reduction = "harmony_theta2", resolution = 0.5)
# run UMAP
