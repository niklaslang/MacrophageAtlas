...
### path variables ###
ramachandran.liver.path <- "/home/s1987963/ds_group/Niklas/ramacha_liver/fibrotic/ramacha_liver_fibrotic_filtered.rds"
harmony.samples.path <- "/home/s1987963/ds_group/Niklas/fibrotic_liver/harmonize_samples/"

### read files ###
liver <- readRDS(ramachandran.liver.path) # 37071 cells
...
for(d in dims){
  ...
  batch1.plot <- DimPlot(liver.harmony, reduction = "umap", group.by = "patient.ID", pt.size = 0.01)
  png(paste0(dim.path, "UMAP_dim", d, ".patient.batch1.png"), width=1600, height=1000, units="px")
  print(batch1.plot)
  dev.off()
  
  # no.2 - split by patients
  batch2.plot <- DimPlot(liver.harmony, reduction = "umap", group.by = "study", split.by = "patient.ID", pt.size = 0.01, ncol = 5)
  png(paste0(dim.path, "UMAP_dim", d, ".patient.batch2.png"), width=2000, height=400, units="px")
  print(batch2.plot)
  dev.off()
  ...
  lineage.overview <- FeaturePlot(liver.harmony, features = lineage.genes, pt.size = 0.5, ncol = 4) & 
    scale_colour_gradientn(colours = rev(brewer.pal(n = 11, name = "RdYlBu")))
...
for(d in dims){
  ...
  for (r in res) {
    ...
    #patient.clustering <- DimPlot(liver.harmony, group.by = "seurat_clusters", split.by = "patient.ID", pt.size = 0.1, ncol = 5)
