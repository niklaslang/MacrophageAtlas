...
DA.path <- "/home/s1987963/ds_group/Niklas/all_blood/DA/MP_figures/"
...
cell.data <- data.table(barcode = colnames(blood),
                        ...
                        lineage = blood$lineage_integrated)
...
blood <- AddMetaData(blood, cell.data, col.name = "celltype_ordered")

# reorder lineage factors
blood$celltype_ordered <- factor(blood$celltype_ordered)
...
#df.celltypes$Celltype <- factor(df.celltypes$Celltype, levels(df.celltypes$Celltype)[c(8,7,6,5,4,3,2,1)])
...
df.MP.fibrotic <- rbind(df.MP.fibrotic, df.MP.fibrotic)
df.MP.fibrotic <- rbind(df.MP.fibrotic, df.MP.fibrotic)
df.MP.fibrotic$Pair.ID <- rep(1:16, each = 8)
df.MP.fibrotic$Pair.ID <- paste0(df.MP.fibrotic$Pair.ID, "_", rep(1:8, 16))
...
df.MP.healthy <- df.MP.healthy[c(seq(1,128,1)),]
df.MP.healthy$Pair.ID <- rep(1:16, each = 8)
df.MP.healthy$Pair.ID <- paste0(df.MP.healthy$Pair.ID, "_", rep(1:8, 16))
...
# plot UMAP
MP.celltype.umap <- DimPlot(blood, reduction = "umap", group.by = "celltype_ordered", cols = c(seurat.colours,"#999999"), label = TRUE, repel = TRUE, pt.size = 0.1) +
...
MP.celltype.boxplot <- ggplot(abundance.pairs, aes(x = log.ratio, y = Celltype, fill = Celltype)) +
  ...
  geom_segment(aes(x=-0.5, y=8.5, xend=-3, yend=8.5), lineend = "round", linejoin = "mitre", arrow=arrow(), size=0.1) + 
  geom_segment(aes(x=0.5, y=8.5, xend=3, yend=8.5), lineend = "round", linejoin = "mitre", arrow=arrow(), size=0.1) +
  ...
  theme(axis.text.x = element_text(size = 12),
        ...
        legend.position="none") +
  geom_text(x = 4, y = 6.90, label = "*", size = 10) +
  geom_text(x = 2, y = 3.90, label = "*", size = 10) +
  geom_text(x = 3.5, y = 2.90, label = "*", size = 10) +
  geom_text(x = 2, y = 1.90, label = "***", size = 10) +
  geom_text(x = -1.5, y = 0.90, label = "*", size = 10) +
