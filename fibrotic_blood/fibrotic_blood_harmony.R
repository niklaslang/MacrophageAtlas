...
### path variables ###
ramachandran.blood.path <- "/home/s1987963/ds_group/Niklas/ramacha_blood/ramacha_blood_fibrotic_filtered.rds"
harmony.samples.path <- "/home/s1987963/ds_group/Niklas/fibrotic_blood/harmonize_samples/"

### read files ###
blood <- readRDS(ramachandran.blood.path) # 28835 cells
...
# harmonize samples
blood.harmony <- blood %>% RunHarmony("patient.ID", theta = 2, reduction.save = "harmony_theta2", plot_convergence = TRUE) # harmonize all 4 samples
...
### save data ###
saveRDS(blood.harmony, paste0(harmony.samples.path, "fibrotic_blood_harmony.rds"))
...
for(d in dims){
  ...
  batch2.plot <- DimPlot(blood.harmony, reduction = "umap", group.by = "study", split.by = "patient.ID", pt.size = 0.01, ncol = 4)
  ...
  lineage.overview <- FeaturePlot(blood.harmony, features = lineage.genes, pt.size = 0.5, ncol = 4) & 
    scale_colour_gradientn(colours = rev(brewer.pal(n = 11, name = "RdYlBu")))
...
for(d in dims){
  ...
  for (r in res) {
    ...
    patient.clustering <- DimPlot(blood.harmony, group.by = "seurat_clusters", split.by = "patient.ID", pt.size = 0.1, ncol = 5)
...
blood.harmony <- FindNeighbors(blood.harmony, reduction = "harmony_theta2", dims = 1:40)
blood.harmony <- FindClusters(blood.harmony, reduction = "harmony_theta2", resolution = 0.5)
# run UMAP
blood.harmony <- RunUMAP(blood.harmony, reduction = "harmony_theta2", dims=1:40, seed.use=1)
